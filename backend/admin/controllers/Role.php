...
//				$message = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
				...
				$groups = Modules::run('admin/group/get_groupsObj');
...
//                                $delete_btn = '<button type="button" class="btn btn-default btn-sm checkbox-toggle"href="javascript:void(0)" onclick="delete_person('."'".$person->id."'".')"><i class="fa fa-close"></i></button>';
				foreach ($roles as $key => $value) {
					$row[] = "<strong>".humanize($value->role_name).
                                                "</strong><br />".
                                                $value->role_name.'<button type="button" class="btn btn-default btn-sm checkbox-toggle"href="javascript:void(0)" onclick="delete_role('."'".$value->id."'".')"><i class="fa fa-close"></i></button>'.
                                                anchor('/admin/role/delete/'.$value->id, 'X');
	...
	public function create(){
		...
		if($this->input->post()){
			$this->form_validation->set_rules('role_type', 'roleType', 'required');
			$this->form_validation->set_rules('role_name', t('roleName'), 'required');
		...
		$data['active'] = 'navigation';
		$this->template->parentTitle(t('adminActions'));
		$this->make('role/v_edit_role', $data);
	}
